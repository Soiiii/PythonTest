...
cur.execute('SELECT id, guid, sender_id, subject_id, send_at FROM Messages')
messages = dict()
for message_row in cur:
    messages[message_row[0]] = (message_row[1], message_row[2], message_row[3], message_row[4])
print("Loaded messages=", len(messages, "subjects=",len(subjects)))
